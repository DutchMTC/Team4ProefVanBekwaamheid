...
public class TileOccupants : MonoBehaviour
{
    ...
    [SerializeField] private int _health = 30; // Current health
    private float _damageReduction = 0f;
    private bool _hasArmor = false; // Added for armor mechanic

    [Header("Armor Visuals (Player Only)")]
    [SerializeField] private List<GameObject> _armorPlayerVisualsToEnable;
    [SerializeField] private List<GameObject> _armorPlayerVisualsToDisable;

    [Header("Damage Delays")]
    [SerializeField] private float _usableAttackDamageDelay = 0f;
    [SerializeField] private float _chargedAttackDamageDelay = 0f;
    [SerializeField] private float _superchargedAttackDamageDelay = 0f;
    [SerializeField] private float _trapDamageDelay = 0f; // Added for trap damage delay
 
    [Header("UI")]
    [SerializeField] private CharacterHealthUI _healthBarUI;
    ...
    void Awake()
    {
        ...
        _health = _maxHealth; // Initialize current health to max health
    ...
    void Start()
    {
        ...
        if (_healthBarUI != null)
        {
            ...
            _healthBarUI.Initialize(this, _maxHealth, _health, isPlayer);
        }
    ...
    private System.Collections.IEnumerator ApplyDamageAfterDelay(int amount)
    {
        ...
        if (_hasArmor)
        {
            ...
            if (_healthBarUI != null)
            {
                _healthBarUI.UpdateArmorStatus(false);
            }
        ...
        if (reducedDamage > 0)
        {
            // 3a. CALCULATE DELAY
            float delay = 0f;
            if (amount >= 25) { delay = _superchargedAttackDamageDelay; }
            else if (amount >= 15) { delay = _chargedAttackDamageDelay; }
            else if (amount >= 10) { delay = _usableAttackDamageDelay; }
            ...
            if (_healthBarUI != null)
            {
                _healthBarUI.OnHealthChanged(_health);
            }
 
            // 3f. PLAY ANIMATION (if still alive and damage was dealt)
            if (_health > 0) // Check health *after* damage is applied
            {
    ...
    public void ReceiveArmor()
    {
        ...
        if (_healthBarUI != null)
        {
            _healthBarUI.UpdateArmorStatus(true);
        }
    ...
    {
        if (_armorPlayerVisualsToEnable != null)
        {
            foreach (GameObject go in _armorPlayerVisualsToEnable)
            {
                if (go != null) go.SetActive(armorActive);
            }
        }

        if (_armorPlayerVisualsToDisable != null)
        {
            foreach (GameObject go in _armorPlayerVisualsToDisable)
            {
                if (go != null) go.SetActive(!armorActive);
            }
        }
    ...
    {
        if (_trapDamageDelay > 0)
        {
            yield return new WaitForSeconds(_trapDamageDelay);
        }
