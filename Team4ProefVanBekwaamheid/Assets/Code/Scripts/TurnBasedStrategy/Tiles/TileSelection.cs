...
public class TileSelection : MonoBehaviour
{
    ...
    public UnityEvent<TileSettings> OnTileSelected = new UnityEvent<TileSettings>();
      ...
      [SerializeField] private CharacterAnimationController _characterAnimationController; // Added for dash animations
      [SerializeField] private CharacterRotator _characterRotator; // Added for rotation logic
      [SerializeField, Range(1f, 20f), Tooltip("Speed at which the player moves between tiles")]
      private float _moveSpeed = 5f; // Speed for smooth movement
    [SerializeField, Tooltip("Time the player pauses on each tile during movement")]
    private float _tilePauseDuration = 0.2f;
    [SerializeField, Tooltip("Curve defining the player's movement animation between tiles")]
    private AnimationCurve _movementCurve = AnimationCurve.EaseInOut(0, 0, 1, 1); // Default to an ease-in-out curve
    [SerializeField, Tooltip("Duration of the rotation when moving or attacking")]
    private float _actionRotationDuration = 0.3f; // Used for rotations

    // Allows other scripts to modify the movement speed
    public float MoveSpeed
    {
        get => _moveSpeed;
        set => _moveSpeed = Mathf.Clamp(value, 1f, 20f);
    }
    ...
    private bool HandlePathSelection(TileSettings hitTile)
    {
        ...
        if (_currentPath != null && _currentPath.Count > 0)
        {
            ...
            if (pathVisualizer != null)
            {
                ...
                StartCoroutine(MoveAlongPath(_currentPath, _tileOccupants.gameObject, _tileOccupants, UserType.Player, _characterAnimationController));
                return true;
            }
            return true;
        }
    ...
    public System.Collections.IEnumerator MoveAlongPath(List<TileSettings> path, GameObject entityToMove, TileOccupants entityOccupants, UserType currentUserType, CharacterAnimationController animController)
    {
        ...
        for (int i = 1; i < path.Count; i++)
        {
            ...
            if (directionToNextTile != Vector3.zero && _characterRotator != null)
            {
                ...
                yield return StartCoroutine(_characterRotator.RotateTowardsAsync(entityToMove.transform, directionToNextTile, rotatorUserType, _actionRotationDuration));
            }
            ...
            {
                while (Time.time - startTime < journeyLength / _moveSpeed)
                {
                    float distanceCovered = (Time.time - startTime) * _moveSpeed;
                    float fractionOfJourney = distanceCovered / journeyLength;
                    // Apply the curve to the fraction of the journey
                    float curvedFraction = _movementCurve.Evaluate(fractionOfJourney);
            ...
            // Pause on the tile
            if (_tilePauseDuration > 0)
            {
                yield return new WaitForSeconds(_tilePauseDuration);
            }
    ...
    {
        if (_characterRotator != null && _tileOccupants != null && targetTile != null && targetTile.tileOccupant != null)
        {
            ...
            yield return StartCoroutine(_characterRotator.RotateTowardsTargetAsync(_tileOccupants.transform, targetTile.tileOccupant.transform, rotatorAttackerUserType));
        }
